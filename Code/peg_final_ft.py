...
dataset_cnndailymail = load_dataset("therapara/summary-of-news-articles")
...
def prepare_data(data_samples):
    cleaned_texts = [cln_text(text) for text in data_samples['document']]
    ...
    with tokenizer.as_target_tokenizer():
        tokenized_labels = tokenizer(data_samples['summary'], max_length=max_target_length)

    tokenized_inputs['labels'] = tokenized_labels['input_ids']
    return tokenized_inputs

tokenized_datasets = dataset_cnndailymail.map(preprocess_data, batched=True)


batch_size = 4
model_name = 'pegasus-large-ft'
